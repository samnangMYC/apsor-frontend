...
import { fetchProviderOrders, updateAdminOrderStatus } from "../../api";

const PROVIDER_ORDER_DEFAULT_SORTING = [{ id: "id", desc: true }];

function getProviderOrderText(lang) {
  return {
    orders: lang === "km" ? "ការបញ្ជាទិញរបស់អតិថិជន" : "Customer Orders",
    subtitle: lang === "km"
      ? "គ្រប់គ្រងការបញ្ជាទិញរបស់អតិថិជនសម្រាប់សេវាកម្មរបស់អ្នក។"
      : "Review customer orders for your services.",
    searchPlaceholder: lang === "km" ? "ស្វែងរកការបញ្ជាទិញរបស់អតិថិជន" : "Search customer orders",
...
function mapOrderSortingToApiQuery(sorting) {
  const currentSort = Array.isArray(sorting) && sorting.length ? sorting[0] : PROVIDER_ORDER_DEFAULT_SORTING[0];
  return {
...
export default function ProviderOrdersPage() {
  ...
  const text = useMemo(() => getProviderOrderText(lang), [lang]);
  const [orders, setOrders] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  const [searchValue, setSearchValue] = useState("");
  const [debouncedSearchValue, setDebouncedSearchValue] = useState("");
  const [sorting, setSorting] = useState(PROVIDER_ORDER_DEFAULT_SORTING);
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      const result = await fetchProviderOrders(orderQuery);
  ...
  const handleSortingChange = useCallback((updater) => {
    setSorting((current) => {
      ...
      return nextSorting.length ? nextSorting : PROVIDER_ORDER_DEFAULT_SORTING;
    });
  ...
  const handleStatusChange = useCallback(async (order, nextStatus) => {
    ...
    try {
      ...
      console.error("Failed to update provider order status:", error);
      showToast(
        "error",
